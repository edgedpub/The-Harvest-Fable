...
pubcoder.page.id = pubcoder.page.id || 859;
...
initAnimation_862();
function initAnimation_862()
{
    var targetObjectId = 862;
    ...
    window.audio_animation_obj862 = $("#")[0];
    window.hasAudioTrack_obj862 = false
    initAnimation(targetObjectId,widthSCAnimationObject,heightSCAnimationObject,framesPerSecond,window.obj862_animation_count,isAnimationInfinite,howManyLoops,backToFirstFrame);
}
	/*
	 *
	 *   Init Shake
...
obj862_onTap_actionGroup0 = function(){
	...
	if (isLastActionGroup) {
		...
		for (var i = 0; i < pubcoder.queuedEvents.length; i++) {
			...
			if (evt.senderObjectId == 862) {
				console.warn("de-queueing event obj862." + evt.eventName);
				pubcoder.queuedEvents.splice(i, 1);
				$("#obj862").trigger(evt.eventName);
				return;
			}
		}
		return;
...
//	target: obj862 
playAnimation_869();
function playAnimation_869() {
	var targetObjectId = "#obj862";
...
if (window.obj862_onTap_runningActionsCount < 0) {
	window.obj862_onTap_runningActionsCount = 0;
} else if (window.obj862_onTap_runningActionsCount == 0) {
	...
	if (window.hasAudioTrack_obj862 && !$(targetObjectId).hasClass("playing")) {
		var myAudio = window.audio_animation_obj862;
	...
	var delayedStartAnimationFuction = function (delayMs) {
		setTimeout(function() {
			var animationObjectApi = window.api_animate_obj862;
	...
	if (!isAnimationInfinite) {
		$("#obj862").one("obj862_animation_ended",function(event) {
		    window.obj862_onTap_runningActionsCount = window.obj862_onTap_runningActionsCount - 1;
if (window.obj862_onTap_runningActionsCount < 0) {
	window.obj862_onTap_runningActionsCount = 0;
} else if (window.obj862_onTap_runningActionsCount == 0) {
...
$("#obj862").bind(PubCoder.Events.Tap, function(event) {
	...
	if (window.obj862_onTap_activeActionGroupIndex != -1) {
	console.warn("action list window.obj862_onTap is still running");
	return;
}
var obj862_onTap_runningActionsCount = 0;
var obj862_onTap_loopCount = 0;
